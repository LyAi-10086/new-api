...
本次增强已把原先“趋势指标更全面”的建议先落到第一版页面里：

- `summary` 新增总 Token、平均耗时、流式请求数、流式占比、错误率。
- `trends` 新增输入 Token、输出 Token、总 Token、登录次数、登录用户数、平均耗时、流式请求数、流式占比、错误率。
- 前端总览卡片新增 Token、登录、平均耗时、流式请求、错误率。
- 趋势表格新增总 Token、登录、平均耗时、流式请求、流式占比、错误率列。

当前仍保持只读统计，不改充值、计费、转发和日志写入流程。
...
- 趋势统计已实现：按小时或天聚合消耗额度、请求数、Token、活跃用户数、登录、平均耗时、流式请求、错误数、充值金额、充值额度、注册用户数。
...
- 当前余额、历史消耗、总请求数等指标尚未全部在页面上显性展示，后续可以继续扩展总览卡片或折叠指标面板。
