...
本轮先落地最小闭环，不一次性铺完整观测大页面，避免和上游后续更新产生过大冲突。

已完成：

- 在模型元数据里增加 `display_name`、`display_order`、`availability_enabled`。
- 模型管理抽屉支持设置用户侧展示名称、排序和是否参与可用性展示。
- 模型管理列表展示用户侧展示名称和可用性展示状态。
- 模型广场、模型详情、性能健康面板优先展示 `display_name`，真实调用、复制模型名、计费和日志仍使用 `model_name`。
- `/api/perf-metrics/summary` 按模型元数据过滤用户侧可见模型，并按 `display_order` 排序。
